...
• Keyboard layout support — Choose US, UK, German, French, Spanish, Italian, Portuguese, Dutch, Canadian English, Australian/NZ, or Irish so symbols and accented characters type correctly on non-US VMs, including proper dead-key composition (e.g. Spanish á, ñ) the same way a real keyboard works.
• Settings sync — Preferences (timing, keyboard layout, panel position, etc.) sync across your devices via the browser's own built-in account sync when signed in — never snippets or clipboard content.
• Optional GitHub Gist backup — Back up snippets to your own GitHub Gist, manually or with an auto-sync toggle. Which snippets get backed up is opt-in per snippet and off by default, since your own token never leaves your device.
• Optional "Enter" after paste — Toggle in Settings to send Enter after each paste.
• Export & import — Backup or move your snippets as a JSON file, or import a shared snippet set from a public Gist.
